...
def write_fasthenry_input(
    ...
    with out_path.open("w", encoding="utf-8") as f:
        f.write("* FastHenry input file created using FreeCAD's ElectroMagnetic Workbench\n")
        f.write("* See http://www.freecad.org, http://www.fastfieldsolvers.com and http://epc-co.com\n\n")
        ...
        for idx, (n1, n2, w_val, h_val) in enumerate(segments):
            ...
            f.write(
                f"{elem_name} {n1} {n2} w={format_coord(w_val)} h={format_coord(h_val)}\n"
            )

        f.write("\n* Ports\n")
        for n_start, n_end in ports:
            f.write(f".external {n_start} {n_end}\n")

        f.write(
            f"\n.freq fmin={format_coord(freq_min, True)} "
